...
app.post("/login.html", function (request, response) {
    ...
    if(typeof users_reg_data[the_username] != 'undefined'){   //To check if the username exists in the json data
        if( users_reg_data[the_username].password ==request.body.password){
            ...
            response.send('Invalid Login: Please hit the back button and try again');
         
        }
        
    }
    if (response.send('Invalid Login')){
      response.redirect('/registration.html?') //Send back to login page if error
     }
});
...
app.get("/registration.html", function (request, response) {
 ...
 //Full Name: Only allows letters and 30 characters max
